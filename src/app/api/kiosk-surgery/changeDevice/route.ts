        ...
        if (deviceResult?.length === 0) {
            ...
            if (deviceResult?.length > 0) {
                ...
                if (deviceResult?.length == 0) {
                    ...
                    const updateSql = `UPDATE KIOSK_SURGERY SET STATUS=${status}, updatedAt=SYSDATETIME() WHERE DEVICE_HASH='${deviceID}' AND PSENTRY='${psEntry}' AND OPDATE='${today}' AND OPCODE='${opCode}'`;
