    ...
    try {
        ...
        if (deviceResult?.length > 0) {
            ...
            if (deviceResult?.length > 0) {
                ...
                if (userId) {
                    const doctorSql = `SELECT * FROM tsfmc_mailsystem.dbo.MAIL_ADMIN WHERE USER_ID = '${userId}'`;
                ...
                return NextResponse.json({
                    success: false,
                    message: "이미 등록된 키오스크 기기지만, 아직 관리자가 사용 승인하지 않았습니다.",
                });
            }
        } else {
            ...
            return NextResponse.json({
                success: false,
                message: "키오스크 등록했습니다. 관리자 등록이 있을 때까지 기다려 주세요.",
            });
        }
    } catch {
        return NextResponse.json(
